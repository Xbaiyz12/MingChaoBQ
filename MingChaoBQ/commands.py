...
async def general_handler(bot: Bot, ev: Event):
    ...
    if text in ("更新索引", "开启白名单", "关闭白名单", "查看白名单", "查看黑名单"):
        return
    if text.startswith("添加白名单") or text.startswith("移除白名单"):
        return
    if text.startswith("添加黑名单") or text.startswith("移除黑名单"):
        return
    if text.startswith("设置戳一戳角色"):
        return
...
@mcbq_sv.on_command("设置戳一戳角色", to_ai="设置本群戳一戳触发的角色（需权限）")
